...
extension SubscriptionsWorker {
    func v3CommerceTools() -> [Tool] {
        [
            ...
            simpleTool("subscriptions_set_price", "Set a subscription's price across ALL territories in one request, equalized from a base price point (like the ASC UI). Pass any single price point (e.g. the USA $1.99 point); the tool fetches every equalized territory point and applies them in one PATCH.", ["subscription_id": "Subscription ID", "price_point_id": "Base subscription price point ID (any territory; USA recommended) — its equalizations define every territory's price", "preserve_current_price": "Optional bool; keep existing subscribers on their current price (default false)", "start_date": "Optional ISO date for the price change"], ["subscription_id", "price_point_id"]),
            simpleTool("subscriptions_set_intro_offer", "Set an introductory offer across ALL available territories in one request (replaces any existing intro offers). Use for a worldwide free trial.", ["subscription_id": "Subscription ID", "duration": "Offer duration (e.g. THREE_DAYS, ONE_WEEK, ONE_MONTH)", "offer_mode": "Offer mode (FREE_TRIAL, PAY_AS_YOU_GO, PAY_UP_FRONT)", "number_of_periods": "Number of periods", "base_price_point_id": "For paid modes only: base price point ID to equalize the offer price across territories"], ["subscription_id", "duration", "offer_mode", "number_of_periods"]),
